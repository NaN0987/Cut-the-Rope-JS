...
var msMatchMedia = window.msMatchMedia || window.matchMedia;

// Modification: pointer events used to propagate through elements, but now they can be blocked by elements on top of each other.
// This modification makes #gameContainer have no pointer events if it isn't displaying any menus.
setInterval(() => {

    function test(string) {
        return document.getElementById(string).style.display == "none"
    }

    // console.log(test("menuPanel") && test("levelPanel") && test("levelMenu") && test("gameCompletePanel") && test("startBackground") && test("menuBackground") && test("levelBackground"))

    if (test("menuPanel") && test("levelPanel") && test("levelMenu") && test("gameCompletePanel") && test("startBackground") && test("menuBackground") && test("levelBackground")) {
...
(function () {
    ...
    var app = window.WinJS && WinJS.Application;

    var activation = window.Windows &&
        Windows.ApplicationModel &&
        Windows.ApplicationModel.Activation;

    var applicationData = window.Windows &&
        Windows.Storage &&
        Windows.Storage.ApplicationData &&
        Windows.Storage.ApplicationData.current;

    var roamingSettings = applicationData && applicationData.roamingSettings;
    ...
    // Note: for some reason, commenting out this function entirely instead of
    // leaving it empty makes the settings appear on the left side of the screen (very convenient).
    ...
    // The shit for "share a photo of this to a friend." Do not click the share button until this is
